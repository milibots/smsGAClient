...
### ۳. استودیوی هوشمند آموزش و ثبت الگو (Smart AI Pattern Studio) 🧠
- **آموزش تعاملی به سیستم**: پذیرنده متن هر پیامک بانکی ناشناخته یا جدید را داخل استودیو پیست کرده و با مشخص کردن مبلغ یا انتخاب از پیشنهادات هوشمند سیستم، به برنامه آموزش می‌دهد.
- **تولید خودکار رجکس استاندارد**: موتور استنتاج `SmartPatternInferrer` کلمات مجاور را تحلیل کرده و به صورت خودکار رجکس ایمن با نام‌گذاری `(?<amount>\d+)`، `(?<card>\d{4})` و `(?<balance>\d+)` می‌سازد.
- **تست و اعتبارسنجی بلادرنگ**: در همان لحظه پیامک با موتور `SmsParser` تست شده و پیش‌نمایش دقیق پارامترهای استخراجی (تومان، ریال، کارت) به کاربر نشان داده می‌شود.
- **فعال‌سازی آنی بدون نیاز به آپدیت اپلیکیشن**: با زدن دکمه «ذخیره در برنامه»، الگو بلافاصله در کش دیتابیس برنامه درج شده و از همان لحظه تمام پیامک‌های بعدی این بانک بدون خطا پردازش و فوروارد می‌شوند!
- **خروجی JSON جهت مشارکت**: دکمه کپی کد JSON استاندارد جهت اضافه کردن به پوشه `banks/` گیت‌هاب یا همگام‌سازی با سرور وب.
...
### ۴. راه‌حل هوشمند برای گوشی‌های آیفون (iOS) یا گوشی شخصی (سناریوی گوشی دوم و فورواردر):
اگر سیم‌کارت حساب‌های بانکی شما روی **گوشی آیفون** قرار دارد (به دلیل محدودیت‌های امنیتی اپل امکان خواندن خودکار پیامک‌ها توسط برنامه‌ها در پس‌زمینه iOS وجود ندارد) یا مایل نیستید گوشی شخصی‌تان دائماً به شارژر و اینترنت وصل بماند:
- **راه‌حل پیشنهادی:**
  1. روی گوشی اصلی، یک برنامه فورواردر پیامک (یا اتوماسیون فوروارد SMS) نصب یا فعال می‌کنید تا پیامک‌های بانکی را به صورت خودکار به **یک سیم‌کارت دوم** پیامک کند.
  2. سیم‌کارت دوم را در یک **گوشی اندرویدی دوم (حتی ساده و ارزان‌قیمت)** قرار داده و اپلیکیشن **smsGAClient** را روی آن نصب می‌کنید.
  3. گوشی دوم را در فروشگاه، دفتر کار یا خانه **دائماً به شارژر و اینترنت وای‌فای/دیتا متصل می‌گذارید**.
- **مزیت بزرگ:** شما در طول روز با گوشی شخصی‌تان جابه‌جا می‌شوید، اما گوشی دوم مثل یک **سرور سخت‌افزاری اختصاصی (POS Server)** بدون وقفه کار می‌کند و پیامک‌ها را درجا به وب‌هوک سایت شما می‌فرستد!

### ۵. آموزش گام‌به‌گام در صورت باز نشدن پنجره مجوز پیامک (Manual Permission Guide):
در نسخه‌های جدید اندروید (به‌ویژه اندروید ۱۱ تا ۱۵ و رابط‌های کاربری شیائومی، سامسونگ یا هواوی)، اگر کاربر قبلاً یک‌بار دسترسی را رد کرده باشد، سیستم‌عامل اندروید اجازه باز شدن مجدد پنجره سیستمی را داخل برنامه نمی‌دهد.

در اپلیکیشن دکمه اختصاصی **«اگر پنجره باز نشد: باز کردن تنظیمات گوشی»** تعبیه شده است که مستقیماً صفحه برنامه را باز می‌کند، یا می‌توانید مراحل زیر را دستی بروید:

#### 🔹 در گوشی‌های سامسونگ (Samsung One UI):
1. وارد **تنظیمات گوشی (Settings)** شوید.
2. گزینه **برنامه‌ها (Apps)** را انتخاب کنید.
3. در لیست، برنامه **smsGAClient** را پیدا و باز کنید.
4. وارد بخش **مجوزها (Permissions)** شوید.
5. روی **پیامک‌ها (SMS)** ضربه زده و گزینه **مجاز است (Allow)** را فعال کنید.
6. به برنامه برگردید؛ سیستم به صورت آنی مجوز را تشخیص داده و صفحه به مرحله بعد منتقل می‌شود.

#### 🔹 در گوشی‌های شیائومی، ردمی و پوکو (Xiaomi / Redmi / Poco - MIUI & HyperOS):
1. وارد **Settings** سپس **Apps** و بعد **Manage apps** شوید.
2. برنامه **smsGAClient** را جستجو و باز کنید.
3. وارد **App permissions** شوید و دسترسی **SMS** را روی **Allow** بگذارید.
4. **بسیار مهم برای شیائومی:** به صفحه قبل برگردید و وارد **Other permissions** شوید و این سه گزینه را سبز (Accept) کنید:
   - ✅ **Show on Lock screen** (نمایش اعلان در صفحه قفل)
   - ✅ **Display pop-up windows while running in the background** (اجرای پاپ‌آپ در پس‌زمینه)
   - ✅ **Permanent notification** (اعلان مداوم سرویس)
5. در صفحه مشخصات برنامه، گزینه **Autostart** (شروع خودکار) را روشن کنید و در بخش **Battery saver** حالت را روی **No restrictions** (بدون محدودیت) قرار دهید.
...
### ⚡ نمونه پیاده‌سازی سرورلس با Cloudflare Worker (رایگان، بدون سرور، دیتابیس KV)

اگر سرور یا IP ثابت ندارید، می‌توانید با استفاده از نمونه آماده **Cloudflare Worker** در کمتر از ۲ دقیقه یک سرورلس وب‌هوک با دیتابیس ابری KV، داشبورد زنده و اعلان فوری تلگرام راه‌اندازی کنید:

- **کد کامل و راهنما**: [پوشه `examples/cloudflare-worker/`](examples/cloudflare-worker/)
- **امکانات**: بدون نیاز به VPS، اعتبارسنجی امنیتی HMAC-SHA256 بومی، پایگاه داده کلید-مقدار (KV) برای جلوگیری از تراکنش تکراری، و داشبورد وب زنده.
